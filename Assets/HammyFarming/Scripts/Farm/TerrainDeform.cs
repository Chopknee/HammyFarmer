...
public class TerrainDeform: MonoBehaviour {
	...
	List<HammyFarming.Farm.FarmFieldDeformation> intersectedFields;
	...
	void Awake () {
		...
		intersectedFields = new List<HammyFarming.Farm.FarmFieldDeformation>();
	...
	void Update () {
		if (isOnField) {
			if (rb.velocity.sqrMagnitude > minDefVelSquared) {
				...
				foreach (HammyFarming.Farm.FarmFieldDeformation ffd in intersectedFields) {
	...
	private void FixedUpdate () {
		...
		if (fieldSound != null) {
			...
			} else if (fieldRollingAS.isPlaying) {
				fieldRollingAS.Stop();
			}
		}
	}
	...
	private void OnTriggerEnter ( Collider other ) {
		...
		if (other.tag == "FarmField") {
			...
			HammyFarming.Farm.FarmFieldDeformation ffd = other.gameObject.GetComponent<HammyFarming.Farm.FarmFieldDeformation>();
			if (ffd && !intersectedFields.Contains(ffd)) {
